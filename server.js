...
// Helper: Parse ISO 8601 Duration (e.g. "PT1H23M45S") to seconds
...
// API Route: Fetch YouTube Playlist Details
app.get('/api/playlist', async (req, res) => {
  ...
  if (!apiKey) {
    ...
    return res.status(500).json({ 
      error: 'YouTube API key is not configured on the server. Please add it to your server env variables.' 
    });
  }
  ...
  try {
    ...
    for (let i = 0; i < totalVideos; i += 50) {
      ...
      chunk.forEach(vid => {
        ...
        // Only include playable/available videos
    ...
    // Response mapping the exact schema used in front-end
    res.json({
      id: playlistId,
      title: title,
      creator: creator,
    ...
    console.error('Server Internal Error during Playlist Analysis:', error);
    res.status(500).json({ error: 'Internal server error while analyzing the playlist. Please try again.' });
  }
});
